...
<div class="row">
    <div class="col-lg-12">
        <div class="card custom-card">
            ...
            <form method="POST" action="/updateLevel/{{ $level->id }}" enctype="multipart/form-data">
                @csrf
                <div class="card-body">
                    <div class="basic-form">
                        <div class="form-group">
                            <label class="form-label">الصلاحية </label>
                            <input value="{{ $level->level }}" required type="text" name="level" class="form-control input-default "><br><br>
                        </div>
                        @foreach ($pages as $page)
                            ...
                            <div class="row majors text-right sys{{ $page->systm_id }}  col-lg-12" align="right" dir="rtl">
                                @foreach ($page["operation"] as $sub)
                                    @php $check = $level["operation"]->contains("id",$sub->id); @endphp
                                    <div class="col-lg-4" dir="rtl" style="font-size: .9375rem;">
                                        <input @if($check) checked @endif value="{{ $sub->id }}" name="operations[]" type="checkbox" class="form-check-input major{{ $page->id }}" style="width: 18px; height: 18px; border-radius: 2px;  border: solid #844fc1; border-width: 2px;">
                                        &nbsp;
                                        {{ $sub->operation }}
                                    </div>
                                @endforeach
